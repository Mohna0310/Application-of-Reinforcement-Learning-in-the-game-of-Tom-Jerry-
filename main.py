...
class Environment:
  ...
  def reset(self, deterministic=True):
    if deterministic:
      ...
      generated = False
      while not generated      or abs(fruit_x - player_x) + abs(fruit_y - player_y) < self.grid_size/2:
...
# When the environment is set to have a 5x5 grid, the maximum reward is 8.  You'll find the training will converge to a rolling average of over 6 reward in about 10,000 episodes.  If all the code is implemented above, running the following cell unchanged should be sufficient.
...
MAX_EPSILON = 0.90 # the rate in which an agent randomly decides its action
...
get_ipython().system(' apt install ffmpeg')
